...
ggsave1 <- function(filename, plot, n.clusters=30, type="h", width.multiplier=1) {  # custom ggsave function
  if (type == "h") {
    height = 10
  ...
  ggsave(filename = filename, plot = plot + no_bkg, width = width * width.multiplier, height = height) #saves plot with custom dimensions 
}
...
DimPlotCluster <- function(obj, lbls) { #DimPlot colored by cluster
  ...
  plot <- ggplot(data, aes(x=UMAP1, y=UMAP2, color=other_cluster)) + geom_point(size = 0.25) + theme_umap + ttl + scale_fill_discrete(labels = lbls)
...
makePlots <- function(obj, results.dir, plot.title, width.multiplier=1) {
  ...
  ggsave1(filename=paste0(results.dir, plot.title, "_umap_clusters.pdf"), plot=DimPlotCluster(obj, lbls), n.clusters=n.clusters, type = "u", width.multiplier)
...
cells <- read.csv(paste0(path, "classification_mad_results.csv"), row.names = 1)
cells.with.coords <- read.csv(paste0(path, "!cells_initial.csv"), row.names = 1)

new.rownames <- rownames(cells.with.coords)
for (i in 1:length(new.rownames)) {
  new.rownames[i] <- paste0(new.rownames[i], "-1")
}
rownames(cells.with.coords) <- new.rownames

cells[["umap1"]] <- cells.with.coords$umap1
cells[["umap2"]] <- cells.with.coords$umap2
cellsddqc_cluster <- as.factor(cells$ddqc_cluster)
...
cells.celltypist <- cells[cells$cell_typist_passed_qc == TRUE, ]
...
makePlots(cells.ddqc, path, plot.title = "ddqc_colored_by_celltypist")
makePlots(cells.celltypist, path, plot.title = "celltypist_colored_by_ddqc")
